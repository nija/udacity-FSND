...
class MovieMetadata(object):
    """This class defines a movie metadata object
        ...
        MovieMetadata object with state: \
        title:Movie Title \
        poster_image_url:http://poster_image.url \
        trailer_youtube_url:https://trailer_youtube.url
    """
    ...
    def __repr__(self):
        return "%s %s < title:%s comment:%s poster_image_url:%s "\
    ...
    def __str__(self):
        return "%s object with state: title:%s comment:%s poster_image_url:%s"\
        " trailer_youtube_url:%s" % \
